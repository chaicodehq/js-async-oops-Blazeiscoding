...
export class HaveliSecurity {
  ...
  removeResident(name, passcode) {
    ...
    const index = this.#residents.findIndex((resident) => resident.name === name);
    if (index === -1) {
  ...
  verifyAccess(name) {
    const allowed = this.#residents.some((resident) => resident.name === name);
    this.#accessLog.push({ name, time: new Date().toISOString(), allowed });
